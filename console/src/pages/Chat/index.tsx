...
export default function ChatPage() {
  ...
  const customFetch = useCallback(
    async (data: {
      ...
      if (!allowed.ok) {
        setLcagentConfigError(allowed.message || "");
        setShowModelPrompt(true);
        return new Response(
          JSON.stringify({
            error: "Home LLM not configured",
            message: allowed.message || "首页大模型未就绪",
          }),
          {
            status: 400,
      ...
      const requestBody = {
        ...(biz_params && typeof biz_params === "object" ? biz_params : {}),
      ...
      >
        <Result
          ...
          subTitle={
            ...
            >
              {lcagentConfigError || t("modelConfig.promptMessage")}
            </span>
          }
          extra={[
            <Button
