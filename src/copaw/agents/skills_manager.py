...
def get_working_skills_dir() -> Path:
    ...
    return get_active_skills_dir(uid)
...
def _load_platform_disabled_skill_names() -> set[str]:
    """Platform-disabled shared skill names from LCAgent config file."""
...
def _get_skill_mtime(skill_dir: Path) -> float:
    ...
    Uses two cheap ``stat()`` calls 鈥?no file content I/O.
...
def _collect_skills_from_dir(skills_dir: Path) -> dict[str, Path]:
    """Map skill folder name -> path for each directory containing SKILL.md."""
...
def sync_skills_to_working_dir(
    ...
    shared_opt = get_shared_skills_dir()
    platform_disabled = _load_platform_disabled_skill_names()

    active_skills.mkdir(parents=True, exist_ok=True)

    merged: dict[str, Path] = {}
    merged.update(_collect_skills_from_dir(builtin_skills))
    merged.update(_collect_skills_from_dir(customized_skills))
    if shared_opt is not None and shared_opt.exists():
        for name, path in _collect_skills_from_dir(shared_opt).items():
            if name in platform_disabled:
                continue
            merged[name] = path
    ...
    for skill_name, src_dir in sorted(merged.items()):
        if skill_names is not None and skill_name not in skill_names:
            continue

        dest_dir = active_skills / skill_name
        if dest_dir.exists() and not force:
            if _is_directory_same(dest_dir, src_dir):
                skipped_count += 1
                continue
        ...
        try:
            ...
            logger.debug(
                "Failed to sync skill '%s' to active_skills: %s",
                skill_name,
                e,
            )
...
def ensure_user_active_skills_initialized(user_id: str | None = None) -> None:
    """
...
def _read_skills_from_dir(skills_dir: Path, source: str) -> list[SkillInfo]:
    """Load SkillInfo entries from a directory of skill folders."""
    skills: list[SkillInfo] = []
    if not skills_dir.exists():
        return skills
...
class SkillServiceLCAgent:
    """Per-user skill dirs (users/.../active_skills) for LCAgent isolation.

    Use this static API when routing by ``user_id`` / JWT instead of workspace
    ``SkillService(workspace_dir)``.
    """
    ...
    def disable_skill(name: str, user_id: str | None = None) -> bool:
        ...
        if not skill_dir.exists():
            logger.debug("Skill '%s' not found in active_skills.", name)
            return False
        try:
            shutil.rmtree(skill_dir)
            return True
        except OSError as e:
            logger.debug("Failed to disable skill '%s': %s", name, e)
            return False
    ...
    def create_skill(
        ...
        _validate_skill_content(content)
        skill_name = _normalize_skill_dir_name(name)
        customized = get_customized_skills_dir(uid)
        customized.mkdir(parents=True, exist_ok=True)
        skill_dir = customized / skill_name
        if skill_dir.exists() and not overwrite:
            return None
        with _staged_skill_dir(skill_name) as staged_dir:
            _write_skill_to_dir(
            ...
            _scan_skill_dir_or_raise(staged_dir, skill_name)
            _copy_skill_dir(staged_dir, skill_dir)
        sync_skills_to_working_dir(skill_names=[skill_name], user_id=uid)
        return skill_name
    ...
    def delete_skill(name: str, user_id: str | None = None) -> bool:
        ...
        try:
            ...
            logger.debug("Failed to delete skill '%s': %s", name, e)
            return False
    ...
    def load_skill_file(
        ...
        if source == "customized":
            base_dir = get_customized_skills_dir(uid) / skill_name
        else:
            base_dir = get_builtin_skills_dir() / skill_name
