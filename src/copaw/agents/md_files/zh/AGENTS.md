...
生成文件时，始终使用 `workspaces/default/` 作为输出目录（例如写 `workspaces/default/xxx.pdf` 而非 `xxx.pdf`）。这样文件才能在前端"Skill 生成"页显示。直接放到用户根目录的文件不会出现在文件列表中。
